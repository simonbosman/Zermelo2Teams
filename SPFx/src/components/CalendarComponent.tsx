...
> {
	...
	private getAppChoicesDialog(
		appointmentActions: ActionsEntity[]
	): ShorthandCollection<RadioGroupItemProps> {
	...
	public render() {
		const { events, onActionChange, onReload } = this.props;
		...
		let headerDate: string = `${startDate.toLocaleDateString(
			"nl-NL",
			options
		)} ${startTime}-${endTime}`;
			...
			<div>
				...
				<Dialog
					...
					content={
						<RadioGroup
							vertical
							items={this.getAppChoicesDialog(
								appointmentActions
							)}
